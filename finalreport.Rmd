...
```
### UPDATE VALUES
Above is a comparison between the ROC for the 8GM (in green) and QTR1 (in blue) models. For the 8GM, $latex AUC = 0.7769 (\pm0.0431)$ while for the improved QTR1 model $latex AUC = 0.8032 (\pm0.0403$.
...
hscohort_test <- hscohort[-trainIndex,]
set.seed(101)
predictors <- c('attendance', 'gpa9th', 'ageHS', 'fails9th', 
                'moves', 'suspended', 'retained')
...
model.9GMr <- train(graduated ~ I(attendance*10) + gpa9th +  
                                I(ageHS>180) + moves + suspended + retained,
                   data = hscohort_train,
                   method='glm',
                   tuneLength = 5,
...
These two models are quite similar, and although the model including 9th grade retention is a better predictive model (ANOVA likelihood ratio test $latex p< 0.0001$), we believe that the improvement is slight enough that using the "throughout-the-year" model (9GM-TOY) to track risk at the end of the second and third quarter is defensible (AUC test $latex p=0.297$). 
...
rocobj1 <- plot.roc(roc_reta, 
                    main="Comparing 9GM with Course Failure v. Retention",
                    col="#1c61b6",
                    print.auc=TRUE,
                    legacy.axes=TRUE)
...
sjp.glm(model.9GMt$finalModel, 
        ...
        axisLabels.y = c('Attendance * 10', '9th Grade GPA', 
                         'Overage Entering High School', 'School Changes',
                         'Days Suspended'))
sjp.glm(model.9GMr$finalModel, 
        ...
        axisLabels.y = c('Attendance * 10', '9th Grade GPA', 
                         'Overage Entering High School', 'School Changes',
                         'Days Suspended', 'Retained in 9th Grade'))
```
...
gpa <- rbind(tables2007_2008$course,
             ...
             select(sasid, gpa10th, schoolyear)
gpa$schoolyear <- ifelse(gpa$schoolyear == '2012',
                           '2011_2012',
                           ifelse(gpa$schoolyear == '2013',
                                  '2012_2013',
                                  gpa$schoolyear))
...
fails <- rbind(tables2007_2008$course,
               ...
               tables2011_2012$course) %.%
         filter(grade == 10) %.%
         group_by(sasid, courseno, schoolyear) %.%
         summarize(gpa10th = mean(gpa, na.rm=TRUE)) %.%
         filter(gpa10th <= 1.0)
...
fails <- ungroup(fails) %.%
         group_by(sasid, schoolyear) %.%
         summarize(fails = n()) %.%
         select(sasid, fails, schoolyear)
fails$schoolyear <- ifelse(fails$schoolyear == '2012',
                           '2011_2012',
                           ifelse(fails$schoolyear == '2013',
                                  '2012_2013',
                                  fails$schoolyear))
...
model.10 <- train(graduated ~ I(attendance*10) + gpa10th + 
                             retained9th + retained + 
                             I(ageHS>180 & ageHS<=192) + I(ageHS>192), 
                 data=hscohort_train,
                 method='glm',
                 tuneLength = 5,
                 trControl = trainControl(method = "repeatedcv", number = 10,
                                          ...
                                          seeds = NA),
                 metric='ROC')
model.10t <- train(graduated ~ I(attendance*10) + gpa10th + 
                             retained9th + 
                             I(ageHS>180 & ageHS<=192) + I(ageHS>192), 
                 data=hscohort_train,
                 method='glm',
...
roc_10t <- roc(hscohort_test$graduated,
               predict(model.10t, newdata=hscohort_test, type='prob')$Y,
               auc=TRUE, ci=TRUE)
...
text(0.5, 0.1, labels=paste("p-value =", format.pval(testobj$p.value)), adj=c(0, .5))  
legend("bottomright", legend=c("10GM R", "10GM T"), col=c("#1c61b6", "#008600"), lwd=2)
