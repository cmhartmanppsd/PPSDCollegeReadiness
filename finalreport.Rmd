...
opts_chunk$set(echo=FALSE, 
               ...
               fig.retina=2)
oldimages <- list.files('/Users/jason/code/PPSDCollegeReadiness/figure/', 
                          pattern='*.png$', full.names=TRUE, ignore.case=TRUE)
invisible(sapply(oldimages, file.remove))
```
...
```{r Initialization}
setwd('/Users/jason/code/PPSDCollegeReadiness/')
...
suspension$date_suspended <- as.character(suspension$date_suspended)
theme_set(theme_jb())
```
...
set.seed(101)
# model.2 <- train(graduated ~ I(attendance8th*10) + gpa8th + matnormal + 
...
model.1 <- train(graduated~ I(attendance8th*10) + gpa8th + I(overage>168) +
                            I(suspend8th>=1), 
                 data=hscohort_train,
                 method='glm',
                 tuneLength = 5,
...
sjp.glm(model.1$finalModel, 
        ...
        axisLabels.y = c('Attendance * 10', '8th Grade GPA', 
                         'Overage Entering 8th Grade', 
                         'Suspended at least 1 day.'))
```
...
hscohort_test$predict8th <- predict(model.1, 
                                     newdata = hscohort_test, 
                                     type = 'prob')$Y

hscohort_test$classification <- cut(hscohort_test$predict8th, 
                             breaks=rev(c(1, thresholds$threshold, 0)), 
                             include.lowest=TRUE, 
                             labels=rev(c('On-track', 
                                        as.character(thresholds$classification))))
hscohort_test$classification <- factor(hscohort_test$classification, 
                                levels = rev(levels(hscohort_test$classification)), 
                                ordered=TRUE)
...
suspension8th <- as.data.frame(with(hs_new, prop.table(table(suspend8th>0,classification),1)))
names(suspension8th)[1] <- 'suspension'
suspension8th$suspension <- as.factor(suspension8th$suspension)
levels(suspension8th$suspension) <- c('Has not been suspended', 
                                   'Suspended one or more days')

ggplot(suspension8th, aes(suspension, Freq, fill=classification)) + 
geom_bar(stat='identity', position='stack') + 
scale_y_continuous('', labels = percent_format(), breaks=seq(0,1,.1)) +
scale_x_discrete('') +
scale_fill_manual('',
...
suspension8th <- as.data.frame(prop.table(table(cut(hs_new$suspend8th, 
                                       ...
                                       1))
names(suspension8th) <- c('suspensions', 'classification', 'Freq')
ggplot(suspension8th, aes(suspensions, Freq, fill=classification)) + 
geom_bar(stat='identity', position='stack') + 
scale_y_continuous('', labels = percent_format(), breaks=seq(0,1,.1)) +
...
model.qtr1 <- train(graduated ~ I(attendance8th*10) + gpa8th + I(overage>168) +
                      gpa_qtr1_9th*fails + I(suspend8th>=1), 
                 data=hscohort_train,
                 method='glm',
                 tuneLength = 5,
                 trControl = trainControl(method = "repeatedcv", number = 10,
...
# model.qtr1_2 <- train(graduated ~ I(attendance8th*10) + gpa8th + I(overage>168) +
#                       gpa_qtr1_9th*fails + I(suspend8th>=1) + 
#                       I(suspended_qtr1>=1),
#                       data=hscohort_train,
#                       method='glm',
...
sjp.glm(model.qtr1$finalModel, 
        ...
        axisLabels.y = c('Attendance * 10', '8th Grade GPA', 
                         ...
                         'Course Failing Grades Quarter 1', 'Ever Suspended in 8th',
...
hscohort_test$predict9qtr1 <- predict(model.qtr1, 
                                     newdata = hscohort_test, 
                                     type = 'prob')$Y

hscohort_test$classification <- cut(hscohort_test$predict9qtr1, 
                             breaks=rev(c(1, thresholds$threshold, 0)), 
                             include.lowest=TRUE, 
                             labels=rev(c('On-track', 
                                        as.character(thresholds$classification))))
...
hscohort_predictions$classification <- cut(hscohort_train$predictqtr1, 
                             breaks=rev(c(1, thresholds$threshold, 0)), 
                             include.lowest=TRUE, 
                             labels=rev(c('On-track', 
                                        as.character(thresholds$classification))))
hscohort_train$classification <- factor(hscohort_train$classification, 
                                levels = rev(levels(hscohort_train$classification)), 
                                ordered=TRUE)
...
myControl <-  trainControl(method = "repeatedcv", 
                           ...
                           summaryFunction = twoClassSummary,
                           seeds = NA)
# model.step <- train(hscohort_train[, predictors],
#                     hscohort_train[, c('graduated')],
#                     method='glmStepAIC',
#                     tuneLength = 5,
#                     trControl = myControl,
#                     metric = 'ROC')
...
hscohort_train$classification <- cut(hscohort_train$predict9GMt, 
                             breaks=rev(c(1, thresholds$threshold, 0)), 
                             include.lowest=TRUE, 
                             labels=rev(c('On-track', 
                                        as.character(thresholds$classification))))
...
bh <- subset(hs_new9, race %in% c('Black', 'Hispanic'))
wa <- subset(hs_new9, !race %in% c('Black', 'Hispanic'))
bhm <- as.data.frame(with(subset(bh, sex=='Male'), 
                                 prop.table(table(classification9GMt))))
bhf <- as.data.frame(with(subset(bh, sex=='Female'), 
                          prop.table(table(classification9GMt))))
bhm$sex <- 'Male'
bhf$sex <- 'Female'
bh <- rbind(bhm, bhf)
bh$bh <- 'Black or Hispanic'

wam <- as.data.frame(with(subset(wa, sex=='Male'), 
                          prop.table(table(classification9GMt))))
waf <- as.data.frame(with(subset(wa, sex=='Female'), 
                          prop.table(table(classification9GMt))))
wam$sex <- 'Male'
waf$sex <- 'Female'
wa <- rbind(wam, waf)
wa$bh <- 'Not Black or Hispanic'

race_sex <- rbind(bh, wa)

ggplot(race_sex, aes(sex, Freq, fill = classification9GMt)) + 
geom_bar(stat = 'identity', position = 'stack') + 
# geom_text(aes(label = percent(Freq), y = Freq, ymax = 1), 
#           position = 'stack',
#           fontface = 'bold',
...
scale_fill_manual('',
                  ...
                  breaks=c('Action', 'Warning', 'Watch', 'On-track')) +
facet_wrap(~bh) +
ggtitle('End of 9th Grade Risk by Race and Sex')
...
set.seed(101)
# model.10glm <- train(hscohort_train[, c('attendance', 'gpa10th', 
#                                         'suspended', 'fails',
#                                   'moves', 'age10')],
#                      hscohort_train[, c('graduated')],
#                      method='glmStepAIC',
...
roc_10 <- roc(hscohort_test$graduated, 
              ...
              auc=TRUE, ci=TRUE)
roc_10f <- roc(hscohort_test$graduated,
               predict(model.10f, newdata=hscohort_test, type='prob')$Y,
               auc=TRUE, ci=TRUE)
rocobj1 <- plot.roc(roc_10, 
                    ...
                    print.auc=TRUE,
                    legacy.axes=TRUE)
rocobj2 <- lines.roc(roc_10f, col="#008600")  
testobj <- roc.test(rocobj1, rocobj2)
...
hscohort_predictions$predict10GM <- predict(model.10, 
                                            ...
                                            type = 'prob')$Y
hscohort_predictions$predict10GMf <- predict(model.10f, 
                                             newdata = hscohort_predictions, 
                                             type = 'prob')$Y
...
m1_matrix <- cutoff_matrix(hscohort_predictions, 'predict10GMf', 'graduated')
thresholds10GMf <- data.frame(classification = as.factor(c('Watch', 
                                                          'Warning', 'Action')),
  threshold = c(m1_matrix[which.min(abs(m1_matrix$true_neg_rate - .6)),]$cutoff,
                m1_matrix[which.min(abs(m1_matrix$true_neg_rate - .7)),]$cutoff,
                m1_matrix[which.min(abs(m1_matrix$true_neg_rate - .8)),]$cutoff))
...
# Compute coverage: what proportion of those who fail to graduate are identified
thresholds10GMf$coverage <- NA
coverage <- as.data.frame(table(
  hscohort_predictions$graduated, 
  hscohort_predictions$predict10GMf <= subset(thresholds10GMf, 
                                     classification=='Action')$threshold))
thresholds10GMf[thresholds10GMf$classification == 'Action', ]$coverage <- 
  coverage[coverage$Var2==TRUE & coverage$Var1 == 'N', 'Freq'] / 
  sum(coverage[coverage$Var1 == 'N', ]$Freq)
coverage <- as.data.frame(table(
  hscohort_predictions$graduated, 
  hscohort_predictions$predict10GMf <= subset(thresholds10GMf, 
                                     classification=='Warning')$threshold))
thresholds10GMf[thresholds10GMf$classification == 'Warning', ]$coverage <- 
  coverage[coverage$Var2==TRUE & coverage$Var1 == 'N', 'Freq'] / 
  sum(coverage[coverage$Var1 == 'N', ]$Freq)
coverage <- as.data.frame(table(
  hscohort_predictions$graduated, 
  hscohort_predictions$predict10GMf <= subset(thresholds10GMf, 
                                     classification=='Watch')$threshold))
thresholds10GMf[thresholds10GMf$classification == 'Watch', ]$coverage <- 
  coverage[coverage$Var2==TRUE & coverage$Var1 == 'N', 'Freq'] / 
  sum(coverage[coverage$Var1 == 'N', ]$Freq)
thresholds10GMf$coverage <- round(thresholds10GMf$coverage, digits=3)

hscohort_predictions$classification <- cut(hscohort_predictions$predict10GMf, 
                             breaks=rev(c(1, thresholds$threshold, 0)), 
                             include.lowest=TRUE, 
                             labels=rev(c('On-track', 
                                        as.character(thresholds$classification))))
hscohort_predictions$classification <- factor(hscohort_predictions$classification, 
                                levels = rev(levels(hscohort_predictions$classification)), 
                                ordered=TRUE)
...
hs_new10 <- left_join(hs_new10,
                      subset(first9th, select=c('sasid', 'schoolyear_9')))
...
# Repeated 10th grade
retained <- subset(annual_demos, grade == 10, 
                   select = c('sasid', 'willrepeatgr', 'schoolyear'))
...
hs_new10$retained <- ifelse(is.na(hs_new10$retained),
                            ...
                            hs_new10$retained)
hs_new10 <- left_join(hs_new10, 
                      subset(attendance, grade == 10,
                             select = c('sasid', 'schoolyear', 'attendance',
                                        'tardy', 'suspended')))                    
...
fails <- ungroup(fails) %.%
         group_by(sasid, schyr) %.%
         summarize(fails = n())
fails <- fails[, c('sasid', 'fails', 'schyr')]
names(fails)[which(names(fails)=='schyr')] <- 'schoolyear'
fails$schoolyear <- paste(fails$schoolyear-1, fails$schoolyear, sep='_')
...
mobile1011 <- moves_calc(filter(tables2011_2012$enrollment, grade == 10),
                         gap = 30, enrollby = "2011-10-01")
mobile1011$schoolyear <- '2011_2012'
mobile1112 <- moves_calc(filter(tables2012_2013$enrollment, grade == 10),
                         gap = 30, enrollby = "2012-10-01")
mobile1112$schoolyear <- '2012_2013'

mobile <- rbind(mobile0607, mobile0708, mobile0809, 
                mobile0910, mobile1011, mobile1112)
mobile$sasid <- as.character(mobile$sasid)
hs_new10 <- left_join(hs_new10, mobile)
...
credits <- all_course %.%
           filter(grade %in% c(9, 10))
credits <- credits[, c('sasid', 'courseno', 'type', 'credits', 'schyr')]
rm(all_course)
credits <- credits[!duplicated(credits), ]
credits <- credits %.%
           group_by(sasid, schyr, type) %.%
           summarize(credits = sum(as.numeric(credits), na.rm = TRUE)) 
credits <- credits[, c('sasid', 'schyr', 'type', 'credits')]
credits$type <- ifelse(credits$type == "", 'none_cred',
                       paste(tolower(credits$type), 'cred', sep = '_'))
names(credits)[which(names(credits)=='schyr')] <- 'schoolyear'
credits$schoolyear <- paste(credits$schoolyear-1, credits$schoolyear, sep='_')
...
credits <- credits %.%
           group_by(sasid, type) %.%
           summarize(credits = sum(credits))
credits <- credits[, c('sasid', 'type', 'credits')]
credits <- dcast(sasid ~ type, value.var='credits', sum, data = credits)
credits$total_cred <- apply(credits[, -1], 1, sum)
credits <- mutate(credits,
                  core_cred = math_cred + eng_cred + sci_cred + soc_cred)
...
hs_new10$predict10GMf <- predict(model.10f, newdata = hs_new10, type='prob')$Y
...
hs_new10$classification10GM <- 
  factor(hs_new10$classification10GM, 
         ...
         ordered=TRUE)
hs_new10$classification10GMf <- cut(hs_new10$predict10GMf, 
                                  breaks=rev(c(1, thresholds10GMf$threshold, 0)), 
                                  include.lowest=TRUE, 
                                  labels=rev(c('On-track', 
                                        as.character(thresholds10GMf$classification))))
hs_new10$classification10GMf <- 
  factor(hs_new10$classification10GMf, 
         levels = rev(levels(hs_new10$classification10GMf)), 
         ordered=TRUE)
```

```{r CompareCredit10GM, results='asis'}
heatmap <- as.data.frame(with(hs_new10, 
                              table(classification10GM, classification10GMf)))
ggplot(data = heatmap, 
       aes(classification10GM, classification10GMf, fill=Freq)) +
geom_tile() +
scale_fill_gradient(low="#EFF3FF", high="#08519C") +
geom_text(aes(fill = heatmap$Freq, label = heatmap$Freq))
```

```{r After10GM, results='asis'}
hs1011_10th <- find10thHS(all_years_enr, '2010_2011')
hs1112_10th <- find10thHS(all_years_enr, '2012')
hs1213_10th <- find10thHS(all_years_enr, '2013')
hs_new10 <- left_join(hs_new10, 
                     rbind(hs1011_10th, hs1112_10th, hs1213_10th))
# Combine all of the Hope High Schools
hs_new10$schno_hs_10_long <- with(hs_new10, 
                            ifelse(as.character(schno_hs_10_long) %in% 
                                     c('181', '183'),
                                  '149', as.character(schno_hs_10_long)))

# Combine PAIS with HSTA & PAIS
hs_new10$schno_hs_10_long <- with(hs_new10, 
                            ifelse(as.character(schno_hs_10_long) %in% 
                                     c('173'),
                                  '175', as.character(schno_hs_10_long)))
school_10 <- as.data.frame(with(hs_new10, 
                                prop.table(table(schno_hs_10_long, 
                                                 classification10GMf), 1)))
school_10 <- na.omit(school_10)
school_10 <- filter(school_10, 
                    !schno_hs_10_long %in% c('185', '147', '114', 
                                             '117', '189', '131'))
#school_ms <- filter(school_ms,
#                            !schno_first %in% c('185', '147'))
#school_lookup <- unique(hs_new[, c('schno_ms_long', 'school_ms_long')])
#school_lookup$schno_ms_long <- as.character(school_lookup$schno_ms_long)
#school_ms$schno_ms_long <- as.character(school_ms$school_ms_long)
school_lookup <- data.frame(schno_hs_10_long = as.character(c(139, 149, 150, 
                                                               164, 167, 169, 
                                                               175, 193)), 
                            school_hs_10_long = c('Central', 'Hope', 
                                                   'Mount Pleasant', 'Classical',
                                                   'E-Cubed', 'Alvarez', 
                                                   'Sanchez', 'Career & Tech'),
                            stringsAsFactors = FALSE)
school_10$schno_hs_10_long <- as.character(school_10$schno_hs_10_long)
school_10 <- left_join(school_10, school_lookup)
school_10_seq <- school_10 %.%
                 filter(classification10GMf == 'On-track') %.%
                 arrange(Freq)
school_10$school_hs_10_long <- factor(school_10$school_hs_10_long,
                                      levels = school_10_seq$school_hs_10_long)
ggplot(data = na.omit(school_10), 
       aes(school_hs_10_long, Freq, fill=classification10GMf)) + 
geom_bar(stat='identity', position='stack') + 
scale_y_continuous('', 
                   labels = percent_format(), 
                   breaks = seq(0,1,.1),
                   limits = c(0, 1.05),
                   expand = c(0, 0)) +
scale_x_discrete('HS Attended Longest 10th Grade',
                 labels = substr(x=levels(school_10$school_hs_10_long), 1, 13)) +
scale_fill_manual('',
                  values=c('#0571B0', '#FFCC00', '#FF9942', '#CA0020'),
                  breaks=c('Action', 'Warning', 'Watch', 'On-track')) +
theme(axis.text.x = element_text(angle=90)) +
ggtitle('Classifications by High School After 10th Grade')

```


```{r EleventhGradeModel}
hs0607 <- subset(person, (schoolyear_first=='2006_2007' & grade_first==9) | 
                         (schoolyear_first=='2007_2008' & grade_first==10) | 
                         (schoolyear_first=='2008_2009' & grade_first==11) | 
                         (schoolyear_first=='2009_2010' & grade_first==12))
...
first11th <- all_years_enr %>%
             filter(grade == 11) %>%
             group_by(sasid) %>%
             summarize(enroll_date = min(enroll_date))
first11th$schoolyear <- ifelse(month(first11th$enroll_date) %in% seq(8,12,1),
                               paste(year(first11th$enroll_date), 
                                     year(first11th$enroll_date) + 1, 
                                     sep = "_"),
                               paste(year(first11th$enroll_date) - 1,
                                     year(first11th$enroll_date),
                                     sep = "_"))
...
hscohort <- left_join(hscohort,
                      ...
                      select(sasid, schoolyear))
hscohort <- subset(hscohort, !is.na(schoolyear))
hscohort <- filter(hscohort, schoolyear %in% c('2008_2009',
                                               '2009_2010',
                                               '2010_2011',
                                               '2011_2012'))
# hscohort$schoolyear_9 <- with(hscohort,
...
hscohort <- left_join(hscohort, 
                      attendance %.%
                      filter(grade == 11) %.%
                      select(sasid, schoolyear, attendance,
                             tardy, suspended))
...
gpa <- rbind(tables2007_2008$course,
             ...
             filter(grade == 11) %.%
             group_by(sasid, schyr) %.%
             summarize(gpa11th = mean(gpa, na.rm=TRUE)) %.%
             select(sasid, gpa11th, schyr)
names(gpa)[which(names(gpa)=='schyr')] <- 'schoolyear'
gpa$schoolyear <- paste(gpa$schoolyear-1, gpa$schoolyear, sep='_')
...
mobile0809 <- moves_calc(filter(tables2008_2009$enrollment, grade == 11),
                         gap = 30, enrollby = "2008-10-01")
mobile0809$schoolyear <- '2008_2009'
mobile0910 <- moves_calc(filter(tables2009_2010$enrollment, grade == 11),
                         gap = 30, enrollby = "2009-10-01")
mobile0910$schoolyear <- '2009_2010'
mobile1011 <- moves_calc(filter(tables2010_2011$enrollment, grade == 11),
                         gap = 30, enrollby = "2010-10-01")
mobile1011$schoolyear <- '2010_2011'
mobile1112 <- moves_calc(filter(tables2011_2012$enrollment, grade == 11),
                         gap = 30, enrollby = "2011-10-01")
mobile1112$schoolyear <- '2011_2012'



mobile <- rbind(mobile0809, mobile0910, mobile1011, mobile1112)
mobile$sasid <- as.character(mobile$sasid)
hscohort <- left_join(hscohort, mobile)
hscohort <- left_join(hscohort, gpa)
hscohort <- left_join(hscohort, fails)
hscohort$fails <- with(hscohort, 
                       ifelse(!is.na(gpa11th) & is.na(fails), 0, fails))
...
credits$schoolyear <- paste(credits$schoolyear-1, credits$schoolyear, sep='_')
year_filter <- hscohort[, c('sasid', 'schoolyear')]
names(year_filter) <- c('sasid', 'eleventhyear')
credits <- inner_join(credits, year_filter)
# Now each instance of a student has the first year of tenth grade in tenthyear
credits$schoolyear <- as.numeric(substr(credits$schoolyear, 1, 4))
credits$eleventhyear <- as.numeric(substr(credits$eleventhyear, 1, 4))
credits <- subset(credits, schoolyear <= eleventhyear)
credits <- credits %.%
           group_by(sasid, type) %.%
           summarize(credits = sum(credits)) %.%
           select(sasid, type, credits)
...
model.11  <- train(graduated ~ I(attendance*10) + gpa11th*fails + moves + 
                               I(age11>204), 
                 data=hscohort_train,
                 method='glm',
                 tuneLength = 5,
...
model.11c <- train(graduated ~ I(attendance*10) + gpa11th + 
                               core_cred + I(age11>204), 
                 data=hscohort_train,
                 method='glm',
                 tuneLength = 5,
                 trControl = trainControl(method = "repeatedcv", number = 10,
...
```{r roc11}
hscohort_test <- hscohort_test[, c('sasid', 'attendance', 'gpa11th', 
                                   'age11', 'core_cred',
                                   'graduated', 'moves', 'fails')]
hscohort_test <- na.omit(hscohort_test)
grade_11_comps <- subset(bowersEWS, grade==11)
roc_11 <- roc(hscohort_test$graduated, 
              predict(model.11, newdata=hscohort_test, type='prob')$Y,
              auc=TRUE, ci=TRUE)
roc_11c <- roc(hscohort_test$graduated,
               predict(model.11c, newdata=hscohort_test, type='prob')$Y,
               auc=TRUE, ci=TRUE)
rocobj1 <- plot.roc(roc_11, 
                    main="11GM ROC",
                    col="#1c61b6",
                    print.auc=TRUE,
                    legacy.axes=TRUE)
rocobj2 <- lines.roc(roc_11c, col="#008600")  
testobj <- roc.test(rocobj1, rocobj2)
# lines.roc(roc_reta, col='gray')
points(x=bowersEWS$specificity, 
       y=bowersEWS$sensitivity, col='gray', pch = 20)  
text(0.5, 0.1, labels=paste("p-value =", format.pval(testobj$p.value)), adj=c(0, .5))  
legend("bottomright", legend=c("11GM", "11GM C"), col=c("#1c61b6", "#008600"), lwd=2)

```


```{r 11GMThresholds}
hscohort_predictions <- hscohort_train[, c('sasid', 'attendance', 'gpa11th', 
                                           'age11', 'core_cred',
                                           'graduated', 'moves', 'fails')]
hscohort_predictions <- na.omit(hscohort_predictions)
hscohort_predictions$predict11GM <- predict(model.11, 
                                            newdata = hscohort_predictions, 
                                            type = 'prob')$Y

m1_matrix <- cutoff_matrix(hscohort_predictions, 'predict11GM', 'graduated')
thresholds11GM <- data.frame(classification = as.factor(c('Watch', 
                                                          'Warning', 'Action')),
  threshold = c(m1_matrix[which.min(abs(m1_matrix$true_neg_rate - .6)),]$cutoff,
                m1_matrix[which.min(abs(m1_matrix$true_neg_rate - .7)),]$cutoff,
                m1_matrix[which.min(abs(m1_matrix$true_neg_rate - .8)),]$cutoff))
...
# Compute coverage: what proportion of those who fail to graduate are identified
thresholds11GM$coverage <- NA
coverage <- as.data.frame(table(
  hscohort_predictions$graduated, 
  hscohort_predictions$predict11GM <= subset(thresholds11GM, 
                                     classification=='Action')$threshold))
thresholds11GM[thresholds11GM$classification == 'Action', ]$coverage <- 
  coverage[coverage$Var2==TRUE & coverage$Var1 == 'N', 'Freq'] / 
  sum(coverage[coverage$Var1 == 'N', ]$Freq)
coverage <- as.data.frame(table(
  hscohort_predictions$graduated, 
  hscohort_predictions$predict11GM <= subset(thresholds11GM, 
                                     classification=='Warning')$threshold))
thresholds11GM[thresholds11GM$classification == 'Warning', ]$coverage <- 
  coverage[coverage$Var2==TRUE & coverage$Var1 == 'N', 'Freq'] / 
  sum(coverage[coverage$Var1 == 'N', ]$Freq)
coverage <- as.data.frame(table(
  hscohort_predictions$graduated, 
  hscohort_predictions$predict11GM <= subset(thresholds11GM, 
                                     classification=='Watch')$threshold))
thresholds11GM[thresholds11GM$classification == 'Watch', ]$coverage <- 
  coverage[coverage$Var2==TRUE & coverage$Var1 == 'N', 'Freq'] / 
  sum(coverage[coverage$Var1 == 'N', ]$Freq)
thresholds11GM$coverage <- round(thresholds11GM$coverage, digits=3)

hscohort_predictions$classification11GM <- cut(hscohort_predictions$predict11GM, 
                                   breaks=rev(c(1, thresholds11GM$threshold, 0)), 
                                   include.lowest=TRUE, 
                                   labels = rev(c('On-track', 
                                               as.character(thresholds11GM$classification))))
hscohort_predictions$classification11GM <- 
  factor(hscohort_predictions$classification11GM, 
         levels = rev(levels(hscohort_predictions$classification11GM)), 
         ordered=TRUE)
...
```{r 11GMOutOfSample}
hs_new11 <- first11th %.%
            filter(schoolyear %in% c('2012_2013'))
hs_new11 <- left_join(hs_new11, person)
hs_new11$sasid <- as.character(hs_new11$sasid)
hs_new11 <- subset(hs_new11, !is.na(schoolyear))
hs_new11 <- left_join(hs_new11, 
                      subset(attendance, grade == 11,
                             select = c('sasid', 'schoolyear', 'attendance',
                                        'tardy', 'suspended')))                    
hs_new11 <- mutate(hs_new11, 
                   age11 = age_calc(dob,
                                    as.Date(paste(substr(schoolyear, 1, 4), 
                                                  '09', '01', sep='-')),
                                    units = 'months'))
...
mobile1213 <- moves_calc(filter(tables2012_2013$enrollment, grade == 11),
                         gap = 30, enrollby = "2012-10-01")
mobile1213$schoolyear <- '2012_2013'
mobile1213$sasid <- as.character(mobile1213$sasid)

hs_new11 <- left_join(hs_new11, mobile1213)
hs_new11 <- left_join(hs_new11, gpa)
hs_new11 <- left_join(hs_new11, fails)
hs_new11$fails <- with(hs_new11, 
                       ifelse(!is.na(gpa11th) & is.na(fails), 0, fails))
...
names(credits)[which(names(credits)=='schyr')] <- 'schoolyear'
credits$schoolyear <- paste(credits$schoolyear-1, credits$schoolyear, sep='_')
year_filter <- hs_new11[, c('sasid', 'schoolyear')]
names(year_filter) <- c('sasid', 'eleventhyear')
credits <- inner_join(credits, year_filter)
# Now each instance of a student has the first year of tenth grade in tenthyear
credits$schoolyear <- as.numeric(substr(credits$schoolyear, 1, 4))
credits$tenthyear <- as.numeric(substr(credits$eleventhyear, 1, 4))
credits <- subset(credits, schoolyear <= eleventhyear)
credits <- credits %.%
           group_by(sasid, type) %.%
           summarize(credits = sum(credits))
...
credits <- mutate(credits,
                  core_cred = math_cred + eng_cred + sci_cred + soc_cred)
hs_new11 <- left_join(hs_new11, credits)

hs_new11 <- subset(hs_new11, !schno_first %in% charters)
valid_cases <- hs_new11[, c('sasid', 'attendance', 'gpa11th', 
                            'age11', 'graduated', 'fails', 'moves')]
valid_cases <- na.omit(valid_cases)
hs_new11 <- subset(hs_new11, sasid %in% valid_cases$sasid)
hs_new11$predict11GM <- predict(model.11, newdata = hs_new11, type='prob')$Y
hs_new11$classification11GM <- cut(hs_new11$predict11GM, 
                                   breaks=rev(c(1, thresholds11GM$threshold, 0)), 
                                   include.lowest=TRUE, 
                                   labels = rev(c('On-track', 
                                               as.character(thresholds11GM$classification))))
hs_new11$classification11GM <- 
  factor(hs_new11$classification11GM, 
         levels = rev(levels(hs_new11$classification11GM)), 
         ordered=TRUE)
```
