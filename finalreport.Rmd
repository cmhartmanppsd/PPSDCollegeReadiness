...
```{r Initialization, echo=FALSE, results='hide', warning=FALSE, message=FALSE}
...
```{r 8GMData, echo=FALSE, results='hide', warning=FALSE, message=FALSE}
...
# Current LEP
lep0708 <- tables2006_2007$person_annual[, c('sasid','lep')]
names(lep0708)[2] <- 'lep8th'
hs0708 <- left_join(hs0708, lep0708)

lep0809 <- tables2007_2008$person_annual[, c('sasid','lep')]
names(lep0809)[2] <- 'lep8th'
hs0809 <- left_join(hs0809, lep0809)

lep0910 <- tables2008_2009$person_annual[, c('sasid','lep')]
...
```{r ExampleTableSource, echo=FALSE, results='hide', warning=FALSE, message=FALSE}
...
```{r TransferCompare, results='hide', echo=FALSE}
...
```{r TransferCompareGraphics, echo=FALSE, results='asis', message=FALSE, fig.height=14, fig.width=10}
multiplot(raceplot, gpaplot, mobilityplot, cols=1)
```
...
```{r RunModel ,echo=FALSE, message=FALSE, warning=FALSE}
...
```{r ModelResamples, echo=TRUE}
summary(resamples(list(glm = model.1, ctree = model.2)))
```
```{r ROCData, results='asis', echo=FALSE, message=FALSE, warning=FALSE}
...
rocobj1 <- plot.roc(roc_lm, 
                    main="Comparing glm and ctree models for\n First Time 10th Graders",
                    col="#1c61b6", 
                    print.auc=TRUE, 
                    legacy.axes=TRUE)
...
```
```{r 8GMParam, results='asis', fig.width=10}
...
We determine the true negative rate by predicting the outcomes for our *test* data set, just as we did for our ROC curves. This provides an accurate depiction of out-of-sample, i.e. with future students, performance.

```{r, CutOffMatrix8GM}
m1_matrix <- cutoff_matrix(hscohort_test, 'predict8th', 'graduated')
...
coverage <- as.data.frame(table(
  hscohort_test$graduated, 
  hscohort_test$predict8th <= subset(thresholds, 
...
Although many demographic characteristics are not present in the model, this is not because all groups of students are served equally well. The following figures examine the distribution of graduation probabilities by student groupings such as race, free or reduced price lunch, special education status, and limited English proficiency. These data look at the incoming high school freshman in the fall of 2012 using the 8GM.

```{r NewestCohort8th, echo=FALSE, results='hide'}
...
hs1213 <- na.omit(hs1213)
hs1213$prediction <- predict(model.1, newdata=hs1213, type='prob')$Y
```

```{r Prediction8GMDisagg, results='asis', fig.width=10}
ggplot(data=hs1213, aes(prediction)) + geom_histogram(binwidth=0.1)
ggplot(data=hs1213, aes(race, prediction)) + geom_boxplot()
ggplot(data=hs1213, aes(lep, prediction)) + geom_boxplot()
ggplot(data=hs1213, aes(iep, prediction)) + geom_boxplot()
ggplot(data=hs1213, aes(as.factor(moves), prediction)) + geom_boxplot()
ggplot(data=hs1213, aes(sex, prediction)) + geom_boxplot()
ggplot(data=hs1213, aes(school_first, prediction)) + geom_boxplot()
```
