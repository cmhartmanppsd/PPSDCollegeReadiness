...
rocobj1 <- plot.roc(roc_orig, 
                    main="Comparing glm and ctree models for\n First Time 10th Graders",
                    col="#1c61b6", 
                    print.auc=TRUE, 
                    legacy.axes=TRUE)
rocobj2 <- lines.roc(roc_qtr1, col="#008600")  
testobj <- roc.test(rocobj1, rocobj2)  
text(0.5, 0.1, labels=paste("p-value =", format.pval(testobj$p.value)), adj=c(0, .5))  
legend("bottomright", legend=c("orig", "qtr1"), col=c("#1c61b6", "#008600"), lwd=2)
sjp.glm(model.orig$finalModel)
```
...
roc_lm_tenth <- roc(hscohort_test$graduated, 
                    predict(model.1, newdata=hscohort_test, type='prob')$Y, 
                    auc=TRUE, ci=TRUE)

roc_ctree_tenth <- roc(hscohort_test$graduated, 
                       predict(model.2, newdata=hscohort_test, type='prob')$Y)

rocobj1 <- plot.roc(roc_lm_tenth, 
                    main="Comparing glm and ctree models for\n First Time 10th Graders",
                    col="#1c61b6", 
                    print.auc=TRUE, 
                    legacy.axes=TRUE)
rocobj2 <- lines.roc(roc_ctree_tenth, col="#008600")  
testobj <- roc.test(rocobj1, rocobj2)  
text(0.5, 0.1, labels=paste("p-value =", format.pval(testobj$p.value)), adj=c(0, .5))  
legend("bottomright", legend=c("glm", "ctree"), col=c("#1c61b6", "#008600"), lwd=2)
